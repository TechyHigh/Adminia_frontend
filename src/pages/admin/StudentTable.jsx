    ...
    <div className="space-y-6 animate-fade-in w-full min-w-0">
      ...
      <div className="bg-card border border-border rounded-xl shadow-sm animate-fade-in-up delay-100">
        ...
        <div className="p-4 border-b border-border flex flex-col sm:flex-row justify-between gap-3 relative">
          ...
          <div className="flex gap-2">
            {selectedCourse && (
              <button onClick={() => setSelectedCourse('')}
                ...
                <X className="w-3.5 h-3.5" /> {selectedCourse.length > 15 ? selectedCourse.substring(0, 15) + '...' : selectedCourse}
              </button>
            )}
            <button onClick={() => setShowFilter(!showFilter)}
              className={`flex items-center justify-center gap-2 px-4 py-2 border border-border rounded-xl text-sm font-medium transition-colors ${showFilter || selectedCourse ? 'bg-primary/5 border-primary/20 text-primary' : 'hover:bg-gray-50'}`}>
          ...
          {/* Filter Dropdown */}
          {showFilter && (
            <div className="absolute right-4 top-[calc(100%-4px)] w-72 bg-white border border-border rounded-2xl shadow-2xl z-20 animate-scale-in origin-top-right overflow-hidden">
              ...
              <div className="max-h-64 overflow-y-auto p-2 custom-scrollbar">
                <button onClick={() => { setSelectedCourse(''); setShowFilter(false); }}
                  className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors mb-1 ${!selectedCourse ? 'bg-primary text-white font-bold' : 'hover:bg-gray-100 text-gray-600'}`}>
                  All Courses
                </button>
                {COURSES.map(course => (
                  <button key={course} onClick={() => { setSelectedCourse(course); setShowFilter(false); }}
                    className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors mb-0.5 last:mb-0 ${selectedCourse === course ? 'bg-primary text-white font-bold' : 'hover:bg-gray-100 text-gray-600'}`}>
                    {course}
                  </button>
                ))}
              </div>
